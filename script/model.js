function AtomasModel () {
  ...
  this.checkForFusions = (atom, isDarkPlus = false, flag = false, intermediateResult = false) => {
    ...
    setTimeout(() => {                                    // cработает функция через 300 мс
      ...
      atom.update(adder);                                   // по новому заряду обновляем получаем элемент из таблицы и устанавливаем
  ...
  this.catchClick = (e) => {                      
    ...
    if (!params.preventIntermediateClick && !params.preventClick) this.renderIntermediateClick(e);// если можно кликать тогда вып функцию
  }
